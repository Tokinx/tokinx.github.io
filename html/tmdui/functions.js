var objs = new Array('case','service','join','about');
var obj_p = new Array();
//浏览器窗口大小变换后
$(window).resize(function() {
	obj_p =[];
	objs.forEach(function(e){
		obj_p.push(parseInt(jQuery("#"+e).offset().top));
	});
});
...
$(document).ready(function() {
    ...
    $(window).scroll(function() {
		...
		//导航浮动
        if (s > t) {
            $('.navContainer').addClass('nav_fixed')
        } else {
            $('.navContainer').removeClass('nav_fixed')
        }
		...
		if(s<(obj_p[0]-5)){
			jQuery( document ).find( 'li > a' ).removeClass( 'current' );
			jQuery( '#nav_0 > a' ).addClass( 'current' );
		}else if(	s>(obj_p[0]-5) 	&& s<obj_p[1]){
			jQuery( document ).find( 'li > a' ).removeClass( 'current' );
			jQuery( '#nav_1 > a' ).addClass( 'current' );
		}else if(	s>(obj_p[1]-5) 	&& s<obj_p[2]){
			jQuery( document ).find( 'li > a' ).removeClass( 'current' );
			jQuery( '#nav_2 > a' ).addClass( 'current' );
		}else if(	s>(obj_p[2]-5) 	&& s<obj_p[3]){
			jQuery( document ).find( 'li > a' ).removeClass( 'current' );
			jQuery( '#nav_3 > a' ).addClass( 'current' );
		}else if(s>(obj_p[3]-5)){
			jQuery( document ).find( 'li > a' ).removeClass( 'current' );
			jQuery( '#nav_4 > a' ).addClass( 'current' );
		}else{
	...
	//默认，判断地址栏
	if( location.hash ){
		jQuery( '.nav > li > a[href=' + location.hash + ']' ).addClass( 'current' );
		$("body,html").animate({
	...
	jQuery( document ).on( 'click', '.nav > li > a', function(){
		jQuery( this ).parent().parent().find( 'li > a' ).removeClass( 'current' );
