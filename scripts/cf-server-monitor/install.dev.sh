...
# 开发版：使用 curl 内部 TCP connect 计时，原 install.sh 保持不变。
...
install_deps() {
    step "检查系统依赖组件..."
    # TCP 探测由 curl 的内部计时完成；不再依赖 ping 或 netcat。
...
get_install_url() {
    ...
    printf '%s/install.dev.sh' "$origin"
}
...
# 取 curl 在进程内部记录的 TCP connect 时长。排除 DNS，并禁用代理与用户 curl 配置。
get_tcp_connect_ms() {
    local host="${1:-}"
    local port="${2:-443}"
    local timing
    ...
    [ -n "$host" ] || return 1
    timing=$(curl -q --noproxy '*' -I -sS -o /dev/null \
        --connect-timeout 2 --max-time 2 \
        -w '%{time_namelookup} %{time_connect}' \
        "http://${host}:${port}/" </dev/null 2>/dev/null || true)

    awk -v timing="$timing" 'BEGIN {
        split(timing, fields, /[[:space:]]+/)
        dns = fields[1] + 0
        connect = fields[2] + 0
        if (connect <= 0 || connect < dns) exit
        ms = (connect - dns) * 1000
        if (ms < 1) ms = 1
        printf "%.0f\n", ms
    }'
...
# 统一 4 次 TCP 探测：输出 "RTT LOSS"。
# RTT 为 TCP connect 中位数（不含 DNS），LOSS 为 TCP 建连失败率。
get_probe() {
    local target="${1:-}"
    local count="${2:-4}"
    local port="${3:-443}"
    ...
    local ok=0 values="" i=1 rtt median_val
    while [ "$i" -le "$count" ]; do
        rtt=$(get_tcp_connect_ms "$host" "$port" 2>/dev/null)
        if [ -n "$rtt" ]; then
            ok=$((ok + 1))
            values="$values $rtt"
        fi
    ...
    median_val=$(printf '%s\n' "$values" | awk '
        ...
        END {
            ...
            for (i = 2; i <= n; i++) {
                value = samples[i]
                j = i - 1
                while (j >= 1 && samples[j] > value) {
                    samples[j + 1] = samples[j]
                    j--
                }
                samples[j + 1] = value
            }
            if (n % 2) median = samples[(n + 1) / 2]
            else median = (samples[n / 2] + samples[n / 2 + 1]) / 2
            if (median < 1) median = 1
            printf "%.0f", median
        }
    ')
    [ -n "$median_val" ] || median_val=1
